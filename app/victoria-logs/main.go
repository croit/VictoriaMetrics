...
var (
	...
	insertHttpListenAddrs = flagutil.NewArrayString("insert.httpListenAddr", "TCP address to listen on for data ingestion in addition to those provided with -httpListenAddr . "+
...
func main() {
	...
	if len(insertListenAddrs) > 0 {
		go httpserver.Serve(insertListenAddrs, requestHandlerInsert, httpserver.ServeOptions{
			UseProxyProtocol: insertUseProxyProtocol,
		})
	}
